 ...
 * 
 * This class is a testing script for Validate PAYM tablet acquisition for new
 * customer.
 */

@Test(groups = { "Regression" })
public class PaymTabletAcquisition extends BaseTest {
	...
	public void paymTabletAcquisition(final String planType, final String deliveryAddress, final String postCode,
			final String customerType, final Customer customerObj, final BillingDetails bdObj,
			final PaymentDetails pdObj) {
		logger.debug("Entered into PaymTabletAcquisition.paymTabletAcquisition");
		Order order = new Order();
		ArrayList<String> extras = new ArrayList<>();
		extras.add("No Selection");
		extras.add("Add FULL COVER");
		order.setName("ipad-air-2-16gb-space-grey").
				// setCapacity("64GB").
				setOptions(SHOP_OPTIONS.TABLET).setBuyFromSection(shopMap.PAYM_TABLETS).setCalls(CALLS.NO_SELECTION)
				.setPlan(PLAN.BEST_SELLING).setExtras(extras);
		shopLogic.tabletOrder(order, planType, deliveryAddress, postCode, customerType, customerObj, bdObj, pdObj);
		logger.debug("Exited from PaymTabletAcquisition.paymTabletAcquisition");
	}
