 ...
 * <p>
 * This class is a testing script for Validate PAYG MBB acquisition for new
 * customer.
 */
@Test(groups = {"Regression"})
public class PaygMBBAcquisition extends BaseTest {
    ...
    public void mbbToBasketBestPlanSubscribing(final String planType,
        ...
        logger.debug("Entered into PaygMBBAcquisition" +
				".mbbToBasketBestPlanSubscribing");
        Order order = new Order();
        ArrayList<String> extras = new ArrayList<>();
        extras.add("No Selection");
        extras.add("Add FULL COVER");

        order.setName("osprey-2").
                // setCapacity("64GB").
                        setOptions(SHOP_OPTIONS.MBB).setBuyFromSection
				(shopMap.PAYM_MBB).setCalls(CALLS.NO_SELECTION)
                .setPlan(PLAN.BEST_SELLING).setExtras(extras);
        shopLogic.makeMBBOrder(order, planType, deliveryAddress, postCode,
                customerType, customerObj, bdObj, pdObj);
        logger.debug("Exited from PaygMBBAcquisition.mbbToBasketBestPlanSubscribing");
    }
}
